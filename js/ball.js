...
class Ball {
  ...
  create() {
    ...
    [[128,128],[128,58],[190,100],[190,158],[128,198],[66,158],[66,100]].forEach(([px,py]) => {
      ctx.beginPath();
      for (let i = 0; i < 5; i++) {
        const a = (i * Math.PI * 2 / 5) - Math.PI / 2;
        i === 0 ? ctx.moveTo(px+22*Math.cos(a), py+22*Math.sin(a)) : ctx.lineTo(px+22*Math.cos(a), py+22*Math.sin(a));
      }
  ...
  shoot(dir, power) {
    ...
    this.body.applyImpulse(new CANNON.Vec3(dir.x * power, power * 0.38, dir.z * power));
  }
  ...
  update() {
    ...
    const sc = Math.max(0.2, 1 - h * 0.15);
